...
def main():
    ...
    manager.load_state_dict(torch.load("manager_transformer_final.pth", map_location=device))
    worker.load_state_dict(torch.load("worker_transformer_final.pth", map_location=device))
    manager.eval()
    worker.eval()
