...
def main():
    ...
    for seed in SEEDS:
        ...
        manager.load_state_dict(
            torch.load(f"manager_transformer_{tag}.pth", map_location=device)
        )
        worker.load_state_dict(
            torch.load(f"worker_transformer_{tag}.pth", map_location=device)
        )
        manager.eval()
        worker.eval()
        ...
        htrans_results[seed] = metrics_h
        print(metrics_h)

    agg = {}
    keys = list(next(iter(htrans_results.values())).keys())
    for k in keys:
        vals = np.array([htrans_results[s][k] for s in SEEDS])
        agg[k + "_mean_over_seeds"] = float(vals.mean())
        agg[k + "_std_over_seeds"] = float(vals.std())

    print("\n=== Summary ===")
    print("No-ESS :", metrics_no)
    print("Rule   :", metrics_rule)
    for seed in SEEDS:
        print(f"H-Trans (seed {seed}):", htrans_results[seed])
    print("\nH-Trans aggregated over seeds:")
